        ...
        <base-header class="pb-6">
            <div class="row align-items-center py-4">
                <div class="col-lg-6 col-7">
                    <h6 class="h2 text-white d-inline-block mb-0">{{ $route.params.id ? 'Edit' : 'New' }} Permission</h6>
                    <!--
                    ...
                    <validation-observer ref="observer" v-slot="{ invalid }" tag="form" @submit.prevent="validate">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <base-button type="secondary" native-type="button" @click="() => $router.push('/permissions')">Cancel</base-button>
                            </div>
                        </div>
                    </validation-observer>
                </div>
...
export default {
    ...
    mounted() {
        if (this.$route.params.id) {
            this.$store.dispatch('permissions/details', this.$route.params.id)
                .then(res => {
    ...
    methods: {
        ...
        async submit(data) {
            return this.$store
                .dispatch("permissions/submit", data)
                .then(response => {
                    this.$router.push('/permissions')
                    this.$notify({type: 'success', timeout: 10000, message: 'Permission information saved successfully!'})
                }).catch( err => {
