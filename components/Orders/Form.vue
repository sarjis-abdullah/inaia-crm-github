                ...
                <ValidationObserver ref="observer" v-slot="{ invalid }" tag="form" @submit.prevent="validate">
                    ...
                    <vs-row>
                        <vs-col vs-lg="6" vs-xs="12" vs-sm="6">
                            ...
                            <vs-select class="w-100 mt-4" label="Order Type?" v-model="resource.order_type_id">
                            <vs-select-item
                                :key="index"
                                :value="item.id"
                            ...
                            <ValidationProvider ref="amountProvider" vid="fee" name="Gold Amount" rules="required" v-slot="{ errors }">
                                <vs-input
                                    label="Gold Amount"
                                    placeholder="amount"
                                    ...
                                    :danger-text="errors ? errors[0] : 'Please insert the Gold Amount'"
                    ...
                    <div class="btn-alignment mt-4">
                        ...
                        <vs-button color="dark" type="filled" @click="() => $router.push('/orders')">Cancel</vs-button>
                    </div>
                </ValidationObserver>
            </vs-card>
        </vs-col>
...
export default {
    ...
    async mounted() {
        if (!this.isFetchingTypes) {
            ...
            await this.$store.dispatch('orderTypes/pairs')
                .then(res => {
        ...
        if (this.$route.params.id && !this.isFetchingDetails) {
            ...
            await this.$store.dispatch('orders/details', this.$route.params.id)
                .then(res => {
                    if (res.data && res.data.data) {
