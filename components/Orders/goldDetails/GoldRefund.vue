<template>
<div class="detailsContainer flex-column">
    ...
    <detail-list-item :title="$t('comment')"><div slot="value">{{order.depotName}}</div></detail-list-item>
    <detail-list-item :title="$t('payout')" v-if="order.transactions && order.transactions.length>0"><div slot="value"> <i18n-n :value="calculateCourse()"></i18n-n> €</div></detail-list-item>
    ...
    <detail-list-item :title="$t('credit')"><div slot="value"> <i18n-n :value="order.amount/100"></i18n-n> €</div></detail-list-item>
    </div>
</template>
<script>
