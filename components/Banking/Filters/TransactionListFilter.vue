<template>
  <div>
    ...
    <form class="card-header border-0" v-if="showFilter">
        ...
        <div class="row">
          <div class="col-md displayFlex flex-column align-content-center">
            ...
            <Select :placeholder="$t('lifecycle_status')"
                    ...
                    >
              <Option v-for="option in lifeCycleStatus"
    ...
    <div class="card-header border-0 border-top" v-if="filterIsActive">
      <Badge type="secondary" size="md" style="margin-right:10px" v-if = "selectedStatus" >{{$t(selectedStatus)}}<a class="badgeIcon" @click.prevent="removeStatus()"><i class="fas fa-window-close"></i></a></Badge>
      <Badge type="secondary" size="md" style="margin-right:10px" v-for = "type in selectedType" :key="type">{{$t(type)}}<a class="badgeIcon" @click.prevent="removeType(type)"><i class="fas fa-window-close"></i></a></Badge>
      <Badge type="secondary" size="md" style="margin-right:10px" v-if = "selectedLifeCycleStatus">{{$t(selectedLifeCycleStatus)}}<a class="badgeIcon" @click.prevent="removeLifeCycleStatus()"><i class="fas fa-window-close"></i></a></Badge>
...
import { STATUS,lifecycle_status,TYPES} from '../../../helpers/bankingTransactions';
...
export default {
  ...
  methods:{
    ...
    {
      this.selectedType = this.selectedType.filter(x=>x.name_translation_key!=type);
       if(this.filterIsActive)
      {
        const query = this.quiryBuilder();
        this.$emit('filter',query)
