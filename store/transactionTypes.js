...
export const actions = {
    pairs(context) {
        return this.$axios
            .get(process.env.golddinarApiUrl + '/transactionType?pair')
            .then(res => {
    ...
    fetchList(context, payload) {
        if (!context.state.loading) {
            ...
            return this.$axios.get(`${ process.env.golddinarApiUrl }/transactionType?include=${ payload }`)
                .then(res => {
    ...
    async details(context, payload) {
        return await this.$axios
            .get(`${ process.env.golddinarApiUrl }/transactionType/${payload}?include=transaction_status`)
            .then(res => {
    ...
    submit(context, payload) {
        if (!payload.id) {
            return this.$axios.post(process.env.golddinarApiUrl + '/transactionType?include=transaction_status', payload)
                .then(res => {
            ...
            return this.$axios.put(process.env.golddinarApiUrl + '/transactionType/' + payload.id + '?include=transaction_status', payload)
                .then(res => {
                    return Promise.resolve(res)
                }).catch(err => {
