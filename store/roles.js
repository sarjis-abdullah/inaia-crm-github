...
export const actions = {
    ...
    fetchList(context, payload) {
        if (!context.state.loading) {
            ...
            return this.$axios.get(`/roles?${ payload }`)
                .then(res => {
    ...
    async details(context, payload) {
        return await this.$axios
            .get(`/roles/${payload}?include=role.permissionIds`)
            .then(res => {
    ...
    submit(context, payload) {
        if (!payload.id) {
            return this.$axios.post('/roles?include=role.permissionIds', payload)
                .then(res => {
            ...
            return this.$axios.put('/roles/' + payload.id + '?include=role.permissionIds', payload)
                .then(res => {
                    return Promise.resolve(res)
                }).catch(err => {
