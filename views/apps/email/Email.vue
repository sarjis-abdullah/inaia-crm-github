...
export default {
  ...
  methods: {
    ...
    sortMail(filterCategory) {
      this.$store.dispatch("vuesax/filteredMail", filterCategory);
  ...
  computed: {
    emails() {
      return this.$store.getters["vuesax/getFilteredMails"];
    },
    inboxCount() {
      return this.$store.getters["vuesax/getInboxCount"];
    },
    isMailOpen() {
          return (emailId) => emailId == this.openCurrentId;
    },
    currentEmail() {
            return this.$store.getters['vuesax/getCurrentMail'](this.openCurrentId)
        },
  },
  created() {
    this.$nextTick(() => {
